...
export interface TacticalCard {
  ...
  costArmies: number;
  icon: string;
  effect: 'air_strike' | 'strategic_air_strike' | 'fortify' | 'spy_objective' | 'emergency_recruits' | 'peace_treaty' | 'blitzkrieg';
}
...
export const TACTICAL_CARDS: TacticalCard[] = [
  ...
  },
  {
    ...
    description: 'Carta secreta descoberta ao conquistar a região estratégica sorteada. Exige pelo menos 20 exércitos na origem. Escolha de 20 até todos os exércitos disponíveis; metade do contingente escolhido é perdida como custo da operação e o restante participa de uma batalha normal contra qualquer território inimigo do mapa. A carta não garante a vitória e é consumida após o uso.',
    costArmies: 0,
    icon: 'Plane',
